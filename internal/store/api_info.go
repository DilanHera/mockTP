...
func (s *ApiInfoStore) GetAll() (*[]ApiInfo, error) {
	var resp []ApiInfo
	rows, err := s.db.Query("SELECT id, name, resp, state FROM app_info")
	if err != nil {
		return nil, err
	}
	for rows.Next() {
		var r ApiInfo
		if err := rows.Scan(&r.ID, &r.Name, &r.Resp, &r.State); err != nil {
			return nil, err
		}
		resp = append(resp, r)
	}
...
func (s *ApiInfoStore) Get(name string) (*ApiInfo, error) {
	var resp ApiInfo
	err := s.db.QueryRow("SELECT id, name, resp, state FROM app_info WHERE name = ?", name).Scan(&resp.ID, &resp.Name, &resp.Resp, &resp.State)
	if err != nil {
		return nil, err
	}
	return &resp, nil
...
func (s *ApiInfoStore) GetMany(names []string) (*[]ApiInfo, error) {
	...
	query := fmt.Sprintf(
		"SELECT id, name, resp, state FROM app_info WHERE name IN (%s)",
...
func (s *ApiInfoStore) Create(name string, resp string, state string) (*ApiInfo, error) {
	_, err := s.db.Exec("INSERT OR IGNORE INTO app_info (name, resp, state) VALUES (?, ?, ?)", name, resp, state)
	if err != nil {
		return nil, err
	}
	return s.Get(name)
