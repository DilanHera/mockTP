...
type EncryptLibResponse struct {
	...
	DeveloperMessage string               `json:"developerMessage" validate:"omitempty"`
	ResultData       ResultDataEncryptLib `json:"resultData" validate:"omitempty"`
...
func (p *phx) EncryptLib(input *EncryptLibRequest) (*EncryptLibResponse, error) {
	res := EncryptLibResponse{}
	result, err := p.app.Service.GetApiInfo("encryptLib", &res)
	if result.State == "C" {
		if err != nil {
			return nil, err
		}
